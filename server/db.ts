...
// Total roadmaps ever generated by this user (inactive ones are kept, not
// deleted) — used to cap AI generations per subscription tier.
export async function countRoadmapsGenerated(userId: number) {
  const db = await getDb();
  if (!db) return 0;
  const result = await db
  ...
  await db.insert(subscriptionPlans).values([
    {
      name: "Essential",
      tier: "free",
      priceMonthly: "9.99" as any,
      priceYearly: "99.99" as any,
      features: JSON.stringify(["Self-assessment & readiness score", "Personal AI roadmap (10 milestones)", "1 AI roadmap generation", "AI assistant", "Resources & official links"]),
    },
    {
      name: "Professional",
      tier: "pro",
      priceMonthly: "19.99" as any,
      priceYearly: "199.99" as any,
      features: JSON.stringify(["Everything in Essential", "Deeper roadmap (15 milestones)", "3 AI roadmap generations", "Unlimited AI assistant", "AI CV & portfolio review", "Interview preparation guides"]),
    },
    {
      name: "Premium",
      tier: "premium",
      priceMonthly: "34.99" as any,
      priceYearly: "349.99" as any,
      features: JSON.stringify(["Everything in Professional", "Fullest roadmap (20+ milestones)", "5 AI roadmap generations", "Priority & detailed AI", "AI mock interview practice"]),
    },
  ]);
